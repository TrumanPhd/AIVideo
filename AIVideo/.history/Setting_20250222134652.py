...
# 初始化文本提示
initial_prompt = "创作一个吸引人的5s视频画面场景,描写尽量详细，在画面中间要有一个小动物或者小女孩或者小精灵，要对它的外貌有详细的描写，主角要有夸张或者快速的动作，环境的部分要自然美观，视频提示文本部分不分段用一个完整自然段输出，不超过400词，要求大开脑洞有天马行空的想象，画面要鲜艳脑洞吸引人，用下面格式输出: 英文标题+|||+画面文本提示不分段用一个完整的段落输出 "
